...
pct_change_line_plot_a <- ggplot(data_pct_change_lines, aes(x = as.factor(year), y = pct_change_police, fill = as.factor(year))) +
  geom_line(group = 1) + geom_point() + theme_linedraw() + theme(legend.position = "none") + 
  scale_y_continuous(labels = scales::percent_format(), limits = c(0, 0.08)) + ylab("Average Percent Change in Police") + xlab("Compared Years")
pct_change_line_plot_b <- ggplot(data_pct_change_lines, aes(x = as.factor(year), y = pct_change_general, fill = as.factor(year))) +
  geom_line(group = 1) + geom_point() + theme_linedraw() + theme(legend.position = "none") + 
  scale_y_continuous(labels = scales::percent_format(), limits = c(0, 0.08)) + ylab("Average Percent Change in General") + xlab("Compared Years")

#save plots
pdf("pct_change_line_plot.pdf", width = 9, height = 4)
plot_grid(pct_change_line_plot_a, pct_change_line_plot_b, labels = c("A", "B"))
dev.off()
